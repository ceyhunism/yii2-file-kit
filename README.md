[![Packagist](https://img.shields.io/packagist/dt/ceyhunism/yii2-file-kit.svg)]()
...
```
php composer.phar require ceyhunism/yii2-file-kit
```

or add

```
"ceyhunism/yii2-file-kit": "@stable"
```
...
```php
'fileStorage'=>[
    'class' => 'ceyhunism\filekit\Storage',
    'baseUrl' => '@web/uploads'
    'filesystem'=> ...
        // OR
    'filesystemComponent' => ...    
...
```
There are several ways to configure `ceyhunism\filekit\Storage` to work with `flysystem`.
...
- Create a builder class that implements `ceyhunism\filekit\filesystem\FilesystemBuilderInterface` and implement method `build` which returns filesystem object. See ``examples/``
...
public function actions(){
    return [
           'upload'=>[
               'class'=>'ceyhunism\filekit\actions\UploadAction',
...
public function actions(){
    return [
       'delete'=>[
           'class'=>'ceyhunism\filekit\actions\DeleteAction',
...
public function actions(){
    return [
       'view'=>[
           'class'=>'ceyhunism\filekit\actions\ViewAction',
       ]
    ];
}
```
...
Standalone usage
```php
echo \ceyhunism\filekit\widget\Upload::widget([
    'model' => $model,
    'attribute' => 'files',
    'url' => ['upload'],
    'sortable' => true,
...
echo $form->field($model, 'files')->widget(
    '\ceyhunism\filekit\widget\Upload',
    [
        'url' => ['upload'],
        'sortable' => true,
...
```php
 public function behaviors()
 {
    return [
        'file' => [
            'class' => 'ceyhunism\filekit\behaviors\UploadBehavior',
